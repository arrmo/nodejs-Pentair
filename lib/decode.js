...
//Validate the checksum on the chatter
                                                                                        //, logger
Decode.prototype.checksum = function (chatterdata, counter, packetType, logMessageDecoding, logger) {
    ...
    if (!validChatter) {
        if (logMessageDecoding) {
            ...
            logger.error('Msg# %s   Mismatch on checksum:   %s!=%s   %s', counter, chatterdatachecksum, databytes, chatterCopy);
        }
...
Decode.prototype.isResponse = function (chatter, counter, packetType, logger, packetFields, queuePacketsArr) {
