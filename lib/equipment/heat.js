...
function setSpaSetpoint(setpoint, callback){
  ...
  container.queuePacket.queuePacket(updateHeat);
  var response = 'Request to set spa heat setpoint to ' + setpoint + ' sent to controller'
...
function setSpaHeatmode(heatmode, callback){
  ...
  var response = 'Request to set spa heat mode to ' + container.constants.heatModeStr[heatmode] + ' sent to controller'
  if (callback!==undefined){
    callback(response)
  }
}
...
function setPoolSetpoint(setpoint, callback){
  ...
  container.queuePacket.queuePacket(updateHeat);
  var response = 'User request to update pool heat set point to ' + setpoint + ': ' + updateHeat
...
function setPoolHeatmode(heatmode, callback){
    ...
    var response = 'Request to set pool heat mode to ' + c.heatModeStr[heatmode] + ' sent to controller : ' + updateHeat
    logger.info(response)
    if (callback!==undefined){
      callback(response)
    }
