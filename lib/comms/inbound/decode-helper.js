...
module.exports = function(container) {
    ...
    function isResponse(chatter, counter, packetType) {
        ...
        if (s.logMessageDecoding) logger.silly('   isResponse:  Msg#: %s  chatterreceived.action: %s (10?) == queue[0].action&63: %s ALL TRUE?  %s \n\n', counter, chatter[container.constants.packetFields.ACTION], queuePacketsArr[0][7] & 63, ((chatter[container.constants.packetFields.ACTION] == (queuePacketsArr[0][7] & 63))))
        ...
        if (packetType == 'pump') {
            ...
            tempObj[2] = tempObj[3];
            tempObj[3] = tempDest;
            if (s.logMessageDecoding) logger.silly('Msg# %s  Comparing pump message for match: \n                                      Sent: %s  Received: %s \n                                      Method 1 - Swap bytes: sent (%s) to received (%s): %s \n                                      Method 2\3 - ACK or Status: %s to %s: %s ', counter, queuePacketsArr[0], chatter, tempObj, chatter, tempObj.equals(chatter), queuePacketsArr[0][7], chatter[container.constants.packetFields.ACTION], queuePacketsArr[0][7] == 1 && chatter[container.constants.packetFields.ACTION] == 1)
            ...
            if (queuePacketsArr[0][7] == 1 && chatter[container.constants.packetFields.ACTION] == 1) //Any commands with <01> are 4 bytes.  The responses are 2 bytes (after the length).  The 3rd/4th byte of the request seem to match the 1st/2nd bytes of the response.
            {
            ...
            else if ((queuePacketsArr[0][7] == 7 && chatter[container.constants.packetFields.ACTION] == 7)) //Scenario 3.  Request for pump status.
            {
                return (true)
            } else //no match
            {
        ...
        if (packetType == 'controller') {
            if (chatter[container.constants.packetFields.ACTION] == 1 && chatter[6] == queuePacketsArr[0][7])
            //if an ACK
            {
                return (true)
            }
            //If a broadcast response to request 202 --> 10
            else if ((chatter[container.constants.packetFields.ACTION] == (queuePacketsArr[0][7] & 63))) {
                /*this works because:
            ...
            else if (chatter[container.constants.packetFields.ACTION] == 252 && (queuePacketsArr[0][7]) == 253) {
                return (true)
            }
    ...
    return {
        countChecksumMismatch,
        processChecksum: function(chatter, counter, packetType) {
            ...
            if (checksum(chatter, counter, packetType, s.logMessageDecoding, container.countChecksumMismatch)) {
        ...
        printStatus: function(data1, data2) {
