...
module.exports = function(container) {
    ...
    app.get('/circuit/:circuit/toggle', function(req, res) {
        ...
        var toggleCircuitPacket = [165, container.intellitouch.preambleByte, 16, bottle.container.settings.appAddress, 134, 2, Number(req.params.circuit), desiredStatus];
        queuePacket(toggleCircuitPacket);
        var response = 'User Request to toggle ' + container.circuit.currentCircuitArrObj[req.params.circuit].name + ' to ' + (desiredStatus == 0 ? 'off' : 'on') + ' received';
    ...
    app.get('/spaheat/setpoint/:spasetpoint', function(req, res) {
        ...
        var updateHeat = [165, container.intellitouch.preambleByte, 16, bottle.container.settings.appAddress, 136, 4, Heat.currentHeat.poolSetPoint, parseInt(req.params.temp), updateHeatMode, 0]
    ...
    app.get('/spaheat/mode/:spaheatmode', function(req, res) {
        ...
        var updateHeat = [165, container.intellitouch.preambleByte, 16, bottle.container.settings.appAddress, 136, 4, Heat.currentHeat.poolSetPoint, Heat.currentHeat.spaSetPoint, updateHeatMode, 0]
        queuePacket(updateHeat);
    ...
    app.get('/poolheat/setpoint/:poolsetpoint', function(req, res) {
        var updateHeatMode = (Heat.currentHeat.spaHeatMode << 2) | Heat.currentHeat.poolHeatMode;
        var updateHeat = [165, container.intellitouch.preambleByte, 16, bottle.container.settings.appAddress, 136, 4, parseInt(req.params.poolsetpoint), Heat.currentHeat.spaSetPoint, updateHeatMode, 0]
        queuePacket(updateHeat);
    ...
    app.get('/sendthispacket/:packet', function(req, res) {
        ...
        if (packet[0] == 16 && packet[1] == c.ctrl.CHLORINATOR) {
            ...
            {
                preamblePacket = [165, container.intellitouch.preambleByte]
            }
