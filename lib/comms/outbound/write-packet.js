...
module.exports = function(container) {
    ...
    preWritePacketHelper = function() {
        ...
        {
            if (s.logPacketWrites) logger.silly('preWritePacketHelper: Setting writeQueueActive=false because last message was successfully received and there is no message to send. %s')
            ...
            if (msgWriteCounter.counter === 0) {
                if (s.logPacketWrites) logger.silly('preWritePacketHelper: Ok to write message %s because it has not been written yet', container.queuePacket.first())
                skipPacketWrittenCount.skipPacketWrittenCount = 0
                writePacket()
            } else if (skipPacketWrittenCount.skipPacketWrittenCount === 2) {
                if (s.logPacketWrites) logger.silly('preWritePacketHelper: Ok to write message %s because it has been skipped twice', container.queuePacket.first())
                skipPacketWrittenCount.skipPacketWrittenCount = 0
                writePacket()
            }
            //if we have not processed more than 4 messages, let's delay again.  However, if we do this twice, then skipPacketWrittenCount >= 2 will be processed and we will write the message no matter what
                ...
                if (s.logPacketWrites) logger.silly('preWritePacketHelper: Skipping write packet %s time(s) because we have not processed four incoming messages since the last write. Packet: %s', skipPacketWrittenCount.skipPacketWrittenCount, container.queuePacket.first())
            ...
            else if (bufferArrayOfArrays.length >= 2 || container.receiveBuffer.getBufferToProcessLength() >= 50) {
                //skipPacketWrittenCount>=2;  we've skipped writting it twice already, so write it now.
                skipPacketWrittenCount.skipPacketWrittenCount++
                    if (s.logPacketWrites) logger.silly('preWritePacketHelper: Skipping write packet %s time(s) due to \n1. bufferArrayOfArrays.length>=2: %s (%s) \n2. bufferToProcess.length>=50:  %s (%s)', skipPacketWrittenCount.skipPacketWrittenCount, bufferArrayOfArrays.length >= 2, bufferArrayOfArrays.length, container.receiveBuffer.getBufferToProcessLength() >= 50, container.receiveBuffer.getBufferToProcessLength())
                writePacketTimer.setTimeout(container.writePacket.preWritePacketHelper, '', '150m')
            } else
            //if none of the conditions above are met, let's write the packet
            {
                if (s.logPacketWrites) logger.silly('preWritePacketHelper: Ok to write message %s because no other conditions have been met', container.queuePacket.first())
                skipPacketWrittenCount.skipPacketWrittenCount = 0
                writePacket()
            }
        }
    ...
    postWritePacketHelper = function() {
        ...
        if (msgWriteCounter.counter === 0) {
            ...
            msgWriteCounter.counter++
                if (s.logPacketWrites) logger.debug('postWritePacketHelper: First time writing packet.', msgWriteCounter)
        } else
        ...
        {
            msgWriteCounter.counter++;
            if (s.logPacketWrites) logger.debug('postWritePacketHelper: Try %s.  Wrote: %s ', msgWriteCounter.counter, container.queuePacket.first())
        }
